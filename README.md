...
- **Guided tour** (`js/onboarding.js`) — a short interactive walkthrough (highlighted tooltips pointing at Dashboard, Compare & Analyze, Job Queue, Resumes, Companies, My Links, Favorites, Support, and Profile) shows automatically the first time any account signs in, tracked with an `onboardingSeen` flag on the profile doc so it never auto-shows twice. Click **Help / Tour** in the sidebar anytime to replay it. **Kept in sync on purpose**: every new nav item/view gets its own tour step, and any major new capability added to an existing view gets its step's text updated — see the MAINTENANCE note at the top of `js/onboarding.js`. If a feature request in this README is ever implemented, updating the tour is part of that same change, not a separate follow-up.
...
- **Cover Letters** — a browsable library of every cover letter gapNinja has drafted for a saved application (searchable by company or role), so an old one is easy to find and reuse — view the full text, **Copy** it, or **Download PDF**. Read-only: editing a letter still happens on its original application in the Dashboard. Also has a **reusable template** card: a saved, hand-editable cover letter (starts from a plain-written example with `[Company Name]`/`[Job Title]` placeholders) you can Copy, Download, or Reset — and from **Compare & Analyze**, a "Use my template" button drops it into a new comparison's cover letter field with the company and role filled in automatically.
- **Favorites** — companies whose career page or a specific posting is worth checking again. Add one directly from the Favorites view, or with one click from **Compare & Analyze** ("☆ Add to Favorites" on a result, pre-filled with the company and job link) or the **Job Queue** ("☆ Favorite" on any queued link). Each favorite has a company name, link, and an optional note, with edit/delete and a **Copy link** button.
...
js/ui-*.js                  View logic: dashboard, compare, resumes, companies, links, favorites, tasks, invite, profile, admin
...
users/{uid}/links/{id}          label, url, order (sort position — only set once you first
                                  reorder; unset links sort newest-first), createdAt
                                  (My Links — personal links list)
users/{uid}/favoriteCompanies/{id}  company, link, note, createdAt   (Favorites — companies
...
users/{uid}/meta/profile        name, email, phone, linkedin, skills[], knowledge, experience[]
                                  ...
                                  theme ("green" (default) / "blue" / "purple" — accent color),
                                  mode ("dark" (default) / "light") — both under Appearance on
                                  the Profile page, coverLetterTemplate (the saved reusable
                                  template on the Cover Letters tab)
